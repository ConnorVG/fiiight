...
struct b2TOIInput
{
    ...
    b2Sweep sweepA;
    b2Sweep sweepB;
    float32 tMax = 0;               // defines sweep interval [0, tMax]
}
...
struct b2TOIOutput
{
    ...
    alias e_unknown    = State.e_unknown;
    alias e_failed     = State.e_failed;
    alias e_overlapped = State.e_overlapped;
    alias e_touching   = State.e_touching;
    alias e_separated  = State.e_separated;
...
void b2TimeOfImpact(b2TOIOutput* output, const(b2TOIInput)* input)
{
    ...
    for (;;)
    {
        ...
        version (none)
        {
            ...
            const int32 N = 100;
            float32 dx    = 1.0f / N;
            float32[N + 1] xs = new float32[N + 1];
            float32[N + 1] fs = new float32[N + 1];
